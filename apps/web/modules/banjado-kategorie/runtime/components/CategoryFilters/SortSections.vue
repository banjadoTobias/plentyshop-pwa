...
/**
 * banjado-Kopie von app/components/CategoryFilters/SortSections.vue, registriert
 * per addComponent in modules/banjado-kategorie/index.ts. Einzige Abweichung vom
 * Kern: die Facette startet eingeklappt und oeffnet nur, wenn laut URL eine
 * Auswahl aktiv ist (siehe startsOpen). Alles andere bewusst unveraendert lassen,
 * damit Upstream-Aenderungen leicht nachzuziehen sind.
 */
...
/**
 * Startzustand aus der URL: Preis-Facette offen bei gesetztem min/max, jede
 * andere Facette offen bei angehaktem Wert. Die URL ist auf Server und Client
 * dieselbe, also rendert SSR denselben Zustand wie der Client - kein
 * Hydration-Unterschied. Danach bestimmt allein der Nutzer den Zustand.
 */
const startsOpen = () => {
  if (!props.facet) {
    return false;
  }
  if (facetGetters.getType(props.facet) === 'price') {
    return minPrice.value.length > 0 || maxPrice.value.length > 0;
  }
  return Object.values(models.value).some(Boolean);
};

updateFilter();
open.value = startsOpen();
