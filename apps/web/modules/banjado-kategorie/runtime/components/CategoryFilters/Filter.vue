...
/**
 * banjado-Kopie von app/components/CategoryFilters/Filter.vue (Suchseite),
 * registriert per addComponent in modules/banjado-kategorie/index.ts. Einzige
 * Abweichung vom Kern: die Facette startet eingeklappt und oeffnet nur, wenn
 * laut URL eine Auswahl aktiv ist (siehe startsOpen).
 */
...
// Expliziter Name: der Kern darf "Filter.vue" heissen, weil Nuxt app/components
// von vue/multi-word-component-names ausnimmt; fuer modules/** gilt das nicht.
defineOptions({ name: 'CategoryFiltersFilter' });
...
/**
 * Startzustand aus der URL, siehe SortSections.vue im selben Ordner: Preis
 * offen bei gesetztem min/max, sonst offen bei angehaktem Wert. SSR = CSR.
 */
const startsOpen = () => {
  if (!props.facet) {
    return false;
