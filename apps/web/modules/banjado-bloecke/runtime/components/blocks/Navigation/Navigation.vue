...
<!--
  banjado-Override des Kern-Blocks Navigation (app/components/blocks/Navigation/Navigation.vue).
  Einziger Zweck: Kategorie-Leiste und Mega-Menue-Dropdown auf die Content-Breite (1280px, wie
  BanjadoHero) zentrieren, waehrend die Hintergrundflaeche der Leiste edge-to-edge bleibt.
  Alles andere (Hover, Tastatur, Touch, Router-Close, Mobil-Drawer) ist unveraenderte Kopie.
  Bei Upstream-Updates: git diff gegen den Kern-Block, nur die markierten Stellen sind eigen.
-->
<template>
      ...
      <ul :class="navigationContainerClasses" :style="navigationContainerStyle" @focusout="onNavBlur">
        ...
        <li v-for="(menuNode, index) in categoryTree" v-else :key="index" @mouseenter="onCategoryMouseEnter(menuNode)">
          ...
          <div
            ...
            :class="[
              'hidden @md:grid gap-x-6 grid-cols-4 mx-auto max-w-[1280px] p-6 pt-5 left-0 right-0 outline-none z-sticky max-h-[calc(100vh-300px)] overflow-y-auto',
              'border border-t-0 border-brand-line rounded-b-brand shadow-brand-lg',
...
// --- banjado: Leiste in Traeger (edge-to-edge) und Container (Content-Breite) aufgeteilt ---

/** Traeger der Kategorie-Leiste: Trennlinie ueber die volle Breite, Farben kommen inline aus dem Editor. */
const NAVIGATION_BAR_CLASSES = 'border-b nav-border border-b-solid';

/**
 * Innerer Container der Kategorie-Leiste. Gleiche Breite und gleicher Innenabstand wie das
 * Dropdown (p-6) und die banjado-Bloecke (BanjadoHero: px-4 md:px-6), damit Kategorie-Buttons,
 * Dropdown-Spalten und Seiteninhalt an einer Kante fluchten. Das Editor-Padding links/rechts
 * wird deshalb bewusst nicht mehr angewendet (es wuerde die px-Klassen per Inline-Style
 * ueberschreiben); oben/unten bleibt es wirksam.
 */
const NAVIGATION_CONTAINER_CLASSES = 'mx-auto flex w-full max-w-[1280px] flex-wrap px-4 @md:px-6';
...
const navigationContainerClasses = computed(() => {
  switch (resolvedContent.value.text.textAlignment) {
    case 'center':
      return `${NAVIGATION_CONTAINER_CLASSES} justify-center`;
    case 'right':
      return `${NAVIGATION_CONTAINER_CLASSES} justify-end`;
    default:
      return `${NAVIGATION_CONTAINER_CLASSES} justify-start`;
  }
});
